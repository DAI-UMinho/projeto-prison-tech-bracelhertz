...
public class AnnotationController {
	...
	@PreAuthorize("hasRole('GUARD') or hasRole('MANAGER') or hasRole('NETWORKMAN')")
	@PostMapping("/comment-annotations")
	public ResponseEntity<ApiResponse> saveUserAnnotation(
		...
		try {
			...
			Long annotationPrisonId = annotationDest.getCreatedBy().getPrison().getPrisonId();
			//
			if (String.valueOf(roleUserLogged).equals("[Role [id=0]]")
					|| String.valueOf(roleUserLogged).equals("[Role [id=1]]")) {
