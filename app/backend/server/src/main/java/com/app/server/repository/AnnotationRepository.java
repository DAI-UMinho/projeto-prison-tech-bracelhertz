...
public interface AnnotationRepository extends JpaRepository<Annotation, Long> {
	...
	@Query("SELECT A.createdBy, A.prisonDest, A.title, A.description, A.createdTimestamp, A.lastUpdatedTimestamp FROM annotation A ORDER BY A.createdTimestamp")
	Optional<List<Annotation>> findByPrisonsAnnotationsNet();

	@Query("SELECT A.createdBy, A.prisonDest, A.title, A.description, A.createdTimestamp, A.lastUpdatedTimestamp FROM annotation A WHERE A.prisonDest = ?1 ORDER BY A.createdTimestamp")
	Optional<List<Annotation>> findByPrisonAnnotations(Prison prisonDest);

	@Query("SELECT A.createdBy, A.prisonerDest, A.title, A.description, A.createdTimestamp, A.lastUpdatedTimestamp FROM annotation A, prisoner P WHERE A.prisonerDest = P.prisonerId AND P.prison = ?1 ORDER BY A.createdTimestamp")
	Optional<List<Annotation>> findByPrisonersAnnotations(Prison prison);

	@Query("SELECT A.createdBy, A.prisonerDest, A.title, A.description, A.createdTimestamp, A.lastUpdatedTimestamp FROM annotation A ORDER BY A.createdTimestamp")
	Optional<List<Annotation>> findByPrisonersAnnotationsNet();

	@Query("SELECT A.createdBy, A.prisonerDest, A.title, A.description, A.createdTimestamp, A.lastUpdatedTimestamp FROM annotation A WHERE A.prisonerDest = ?1 ORDER BY A.createdTimestamp")
	Optional<List<Annotation>> findByPrisonerAnnotations(Prisoner prisonerDest);

	@Query("SELECT A.createdBy, A.userDest, A.title, A.description, A.createdTimestamp, A.lastUpdatedTimestamp FROM annotation A WHERE A.userDest = ?1 ORDER BY A.createdTimestamp")
	Optional<List<Annotation>> findByUserAnnotations(User userDest);

	@Query("SELECT A.createdBy, A.annotationDest, A.title, A.description, A.createdTimestamp, A.lastUpdatedTimestamp FROM annotation A WHERE A.annotationDest = ?1 ORDER BY A.createdTimestamp")
