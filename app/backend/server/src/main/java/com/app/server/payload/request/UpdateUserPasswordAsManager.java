...
public class UpdateUserPasswordAsManager {
	...
	public UpdateUserPasswordAsManager(Long userId,
			@NotBlank(message = "Can't be blank") @Pattern(regexp = "^(?=.*\\d)(?=.*[a-z])(?=.*[A-Z])(?!.*\\s).*$", message = "Needs at least 1 UpperCase, 1 LowerCase and 1 Number") String newPassword) {
		super();
		this.userId = userId;
