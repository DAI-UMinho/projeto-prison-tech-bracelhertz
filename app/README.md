...
# Patch Notes 1.1
-Versão SandBox (v0.0; 10/4/2020): Funcionalidades básicas (Tudo menos pulseira e reconhecimento facial)
	-Nesta versão inicial as funcionalidades básicas da aplicação já estão presentes, sendo que, apenas a pulseira em si (juntamente com as ligações respetivas) e o reconhecimento facial não foram implementados.
-Versão MVP (v1.0; 17/4/2020):  Adição da pulseira e sistema de alertas 
	-Nesta segunda versão, a pulseira e as suas funcionalidades (incluindo o sistema de alertas) foram adicionadas hás demais. 
-Versão MVP (v1.1; 24/04/2020): Implementação da PWA
	-Esta versão já conta com todas as funcionalidades primárias para a utilização da aplicação juntamente com a pulseira. Todas as versões já lançadas (incluindo esta), foram concluídas antes da fase da Elaboration.
-Versão Beta (v2.0; 01/05/2020): Aprimoramento da segurança e responsividade (versão para telemóvel)
	-Após a implementação da PWA, a segurança da aplicação foi, mais uma vez, melhorada e a possibilidade de utilizar a aplicação em “mobile” foi adicionada.
-Versão Beta (v2.1; 15/05/2020): Aprimoramento do aspeto visual  
	-Alguns elementos referentes ao aspeto visual da aplicação foram aprimorados de maneira a facilitar a utilização da mesma, temos como exemplo a adição do registo criminal e cuidados médicos no registo do recluso.
-Versão Alfa (v3.0 29/05/2020): Adição do reconhecimento facial, melhoramento do algoritmo da pulseira e melhor display dos alertas
	-Nesta versão final a última funcionalidade da aplicação foi adicionada (reconhecimento facial) e tanto o funcionamento da pulseira como os alertas apresentados relativos a esta sofreram alterações.
